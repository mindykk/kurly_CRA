...
const MemberComponent = ({modal,modalOpen,이용약관}) => {
  ...
  const [field, setField] = useState(
        {
          ...
          추가입력사항선택: '', //라디오 버튼 선택 항목
          isShowAddInput: false, //show,hide
          추가입력사항: '', //추천인 또는 이벤트명 저장
  ...
  //3. 아이디 확인 버튼 클릭 이벤트: 모달창 띄우기
  const onClickIdOk=(e)=>{
    e.preventDefault();
    if(field.아이디===''){
      modalOpen('아이디를 입력하세요.');
    }
    else {
      if(field.isClassId===false){ //정규표현식이 오류가 있다면
        modalOpen('아이디는 6자 이상의 영문 혹은 영문과 숫자를 조합하여 입력하세요.');
      }
      else {
        let imsi = [];
        ...
        if(result.includes(true)){ //중복된 아이디
          modalOpen('중복된 아이디입니다.');
        }
        else {
          modalOpen('사용가능한 아이디입니다.');
        }
      }
    }
  }
  ...
  const onClickEmailOk=(e)=>{
    e.preventDefault();
    if(field.이메일===''){
      modalOpen('이메일을 입력하세요.');
    }
    else {
      if(field.이메일===false){
        modalOpen('잘못된 이메일 형식입니다.');
      }
      else {
        let imsi = [];
        ...
        if(result.includes(true)){ //중복된 이메일
          modalOpen('중복된 이메일입니다.');
        }
        else {
          modalOpen('사용가능한 이메일입니다.');
        }
      }
    }
  }
  ...
  const onClickHp=(e)=>{
    ...
    //모달창 띄우기
    if(field.휴대폰===''){
      modalOpen(`휴대폰 번호를 입력해주세요.`);
    }
    else {
      modalOpen(`휴대폰으로 인증번호(${num})가 전송되었습니다.`);
    }
  }
  ...
  //생년월일 - 공통으로 사용하는 함수
  //키보드 focusin,focusout 이벤트 발생시 호출하는 함수
  const birthDayCheck=()=>{
    //비구조화
    const {생년,생월,생일} = field;
    const regExpYear = /^(?:19[0-9][0-9]|2[0-9][0-9][0-9])$/; //1900~2999
    const regExpMonth = /^(?:0?[1-9]|1[0-2])$/g; // 1~12
    const regExpDate = /^(?:0?[1-9]|1[0-9]|2[0-9]|3[0-1])$/g; //1~31
    ...
    if(생년==='' && 생월==='' && 생일===''){ //세 칸 모두 공백이면 return
      return;
    }
    else {
      if(regExpYear.test(생년)===false){ //가이드 텍스트 보이기
        setField({...field, isShowBirthError: true, isShowBirthText: '태어난 년도 4자리를 정확하게 입력해주세요.'});
        return;
      }
      else { //생년이 정상이면 멤버변수 초기화하고 생월 체크
        ...
        if(regExpMonth.test(생월)===false){
          ...
          if(regExpDate.test(생일)===false){ //생일 체크
          ...
          //1. 미래
          if(birthDay > today){
            setField({...field, isShowBirthError: true, isShowBirthText: '생년월일이 미래로 입력되었어요.'});
            return;
          }
          else {
            setField({...field, isShowBirthError: true, isShowBirthText: ''});
          }
  ...
  const onChangeRadioAddInput=(e)=>{
    setField({...field, isShowAddInput: true, 추가입력사항: e.target.value});
  }

  //추가입력: 추가입력 또는 이벤트 내용 저장
  ...
  const onChangeServiceAll=(e)=>{
    if(e.target.checked){
      setField({...field, 이용약관동의: 이용약관}); //props 이용약관 전체 저장
    }
    else {
  ...
  const onChangeService=(e)=>{
    ...
    else { //체크 해제시 저장된 데이터 삭제 - filter 사용
      if(e.target.value==='무료배송, 할인쿠폰 등 혜택/정보 수신 동의'){
        imsi = field.이용약관동의.filter((item)=> item !== e.target.value); //삭제 1
        imsi = field.이용약관동의.filter((item)=> item !== 'SNS'); //삭제 2
        imsi = field.이용약관동의.filter((item)=> item !== '이메일'); //삭제 3
        setField({...field, 이용약관동의: imsi});
      }
      else if(field.이용약관동의.includes('SNS') && e.target.value==='이메일'){
        imsi = field.이용약관동의.filter((item)=> item !== '이메일'); //삭제 1
        imsi = imsi.filter((item)=> item !== '무료배송, 할인쿠폰 등 혜택/정보 수신 동의'); //삭제 2
        setField({...field, 이용약관동의: imsi});
      }
      else if(field.이용약관동의.includes('이메일') && e.target.value==='SNS'){
        imsi = field.이용약관동의.filter((item)=> item !== 'SNS'); //삭제 1
        imsi = imsi.filter((item)=> item !== '무료배송, 할인쿠폰 등 혜택/정보 수신 동의'); //삭제 2
        setField({...field, 이용약관동의: imsi});
      }
      else {
  ...
  //submmit 전송: 전송버튼 클릭시 동작
  //1. local storage에 저장(전송)
  ...
  const onSubmitMember=(e)=>{
    ...
    if(아이디==='' || 비밀번호==='' || 비밀번호확인==='' || 이름==='' || 이메일==='' || 휴대폰==='' || 주소1==='' || 주소2==='' || 이용약관동의.length<=3 || 아이디중복확인Ok===false || 이메일중복확인Ok===false || 휴대폰인증번호확인Ok===false){
      if(아이디===''){
        modalOpen('아이디를 입력해주세요.');
      }
      else if(비밀번호===''){
        modalOpen('비밀번호를 입력해주세요.');
      }
      else if(비밀번호확인===''){
        modalOpen('비밀번호를 확인해주세요.');
      }
      else if(이름===''){
        modalOpen('이름을 입력해주세요.');
      }
      else if(이메일===''){
        modalOpen('이메일을 입력해주세요.');
      }
      else if(휴대폰===''){
        modalOpen('휴대폰를 입력해주세요.');
      }
      else if(주소1===''){
        modalOpen('주소를 검색해주세요.');
      }
      else if(주소2===''){
        modalOpen('상세주소를 입력해주세요.');
      }
      else if(아이디중복확인Ok===false){
        modalOpen('아이디 중복확인해주세요.');
      }
      else if(이메일중복확인Ok===false){
        modalOpen('이메일 중복확인해주세요.');
      }
      else if(휴대폰인증번호확인Ok===false){
        modalOpen('휴대폰 인증번호를 다시 확인해주세요.');
      }
      return;
    }
    else {
      //전송할 데이터를 임시 배열에 저장하고 localstorage에 임시 배열을 한꺼번에 저장한다
      //약관동의 필수 선택사항 3개 확인하고 만약 3개 미만이면 return
      let cnt = 0;
      이용약관동의.map((item)=>{
        if(item.includes('필수')){ //필수 포함된 배열 데이터가 있으면(true) 카운트 증가
          cnt++;
        }
      });

      if(cnt<3){
        modalOpen(`이용약관동의 필수 항목을 체크(${cnt})해주세요.`);
        return;
      }
      else {
        let imsi = {
        ...
        //localstorage는 데이터 저장시 객체 저장할 수 없다
        //그래서 문자열로 변환(JSON.stringfy())한다.
        localStorage.setItem(imsi.아이디, JSON.stringify(imsi));
        modalOpen('회원가입이 완료되었습니다.');
        setField({...field, 
          아이디: '',
          isShowId: false,
          isClassId: '',
                ...
                <ul id="memberForm">
                    ...
                    <div className="right">
                      {
                        field.isShowAddress && (
                          <>
                            <input onChange={onChangeAddress1} type="text" id="inputAddress1" name="inputAddress1" placeholder="검색주소" value={field.주소1} />
                            <input onChange={onChangeAddress2} type="text" id="inputAddress2" name="inputAddress2" placeholder="세부주소를 입력하세요" value={field.주소2} />
                          </>
                        )
                      }
                  ...
                  <li className="add-input-item">
                    ...
                    <div className="right gender add">
                      <label htmlFor="add1">
                        <input onChange={onChangeRadioAddInput} type="radio" id="add1" name="add" className="add-radio" value="추천인 아이디" />
                        <span>추천인 아이디</span>
                      </label>                    
                      <label htmlFor="add2">
                        <input onChange={onChangeRadioAddInput} type="radio" id="add2" name="add" className="add-radio" value="참여 이벤트" />
                        <span>참여 이벤트</span>
                      </label>
                        {
